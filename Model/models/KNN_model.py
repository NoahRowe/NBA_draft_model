...
# START THE FUNCTION
##################################################################################################
def simulation(scaler_type, n_clusters, distance_metric, n_players, 
               nba_scaler_type, OS_size, US_size, OS_val, US_val, count):

    filenamevals = [str(i) for i in 
                    [scaler_type, n_clusters, distance_metric, n_players, nba_scaler_type, 
                     OS_size, US_size, OS_val, US_val, 80, count]]
    filename = '_'.join(filenamevals)+".npy"
    #filename = "BASE_{}.npy".format(count)
...
#             print("{}/{} completed!".format(i+1, len(scaledDraft)))
    ...
    np.save("./Results/KNN_results/"+filename, results)
...
simulation(scaler_type, n_clusters, distance_metric, n_players, nba_scaler_type, OS_size, US_size, OS_val, US_val, count)
